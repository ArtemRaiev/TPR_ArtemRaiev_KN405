...
$(document).ready(function(){
	...
	// Показуємо обмеження 
	//(зроблено для плавної появи нових обмежень)
	$('.ogranichenie').show();
	
})
	// Показуємо або приховуємо приклад
	$('#info a.example').live('click', function(){
		if ($('#info .left').is(':hidden')){
			$(this).text('Сховати приклад ▲');
			$('#info .left').slideDown(200);
		}else{
			$('#info .left').slideUp(200);
			$(this).text('Показати приклад ▼');
		}
		return false;
	})
	// Добавляємо Х
	$('a.add_x').live('click', function(){
		// Обмежили кількість Іксів до 7, інакше вони не поміщаються в рядок і виглядають негарно
		if ($(this).parents('.virazhenie').find('input').length < 7){ // Обмеження можна зняти або змінити
	...
	// Додамо її в основну матрицю
		matrix.push(Fun); 
	
	// Чи є негативні елементи в матриці вільних членів?
	if (minelm(free) < 0){ 
		iteration = 0; // лічильник ітерацій, для захисту від зависання
		step1(); // Переходимо до кроку 1
	}
	// Чи є негативні елементи в коефіцієнт функції (останній рядок)?
	if (minelm(matrix[matrix.length-1], false, true) < 0){
		iteration = 0; // лічильник ітерацій, для захисту від зависання
		step2(); // Переходимо до кроку 2
	}	
	print_table(matrix); // Відображаємо підсумкову таблицю
	results(); // Відображаємо результати в зрозумілому вигляді
...
function step1(){
		...
		if (minelm(matrix[min_k_num]) < 0){
			...
			alert('Умови завдання несумісні і рішень у неї немає');
			return false;
		}
		// Друкуємо таблицю і виділяємо на ній провідні рядок і стовпець
		print_table(matrix, min_k_num, min_k1_num); 
		// Оновлюємо індекси елементів по горизонталі і вертикалі
	...
	// матриця вільних членів
		...
		if (minelm(free, false, true) < 0 && iteration < 10) // чи потрібно ще разок пройти другий крок?
		if (confirm("продовжуємо Крок 1_"+iteration+" ?")) 
			step1();
...
function step2(){
		...
		horisont_x[min_k1_num] = vertical_x[min_k_num];
		vertical_x[min_k_num] = tmp;
		// Якщо ми не знайшли провідний рядок (999 - це наш емпіричний коефіцієнт)
		if (min_row_num == 999){
			alert('функція в області допустимих рішень задачі не обмежена');
			return false;
		}
	...
	// чи потрібно ще раз пройти 2 крок?	
		if (minelm(matrix[matrix.length-1], false, true) < 0 && iteration < 10) 
	// рекурсія		
		if (confirm("продовжуємо Крок 2_"+iteration+" ?")) 
			step2();

}
// Функція заміни (відновлення матриці)
function update_matrix(min_k_num, min_k1_num){
	...
	// Виводимо результати в зрозумілому вигляді
	function results(){
		var nulls = '';
		// Ікси, рівні нулю
...
function minelm(v, dispnum, not_last){ 
	...
	var num= 0;
	var len=0;
	// якщо not_last, то останній елемент не враховуємо в масиві
	if (not_last){
		len = v.length-2;
	}else{
		len = v.length-1;
